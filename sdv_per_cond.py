...
class PLV:
    ...
    def get_plv(self):
        ...
        self.plv = analyses.compute_sync(complex_signal, mode='plv', epochs_average=True)
...
mom_fp1 = [f for f in os.listdir(thesisPath) if re.match(r"Mother\d+_1\.fif", f)]
mom_sf1 = [f for f in os.listdir(thesisPath) if re.match(r"Mother\d+_2\.fif", f)]
mom_fp2 = [f for f in os.listdir(thesisPath) if re.match(r"Mother\d+_3\.fif", f)]
mom_sf2 = [f for f in os.listdir(thesisPath) if re.match(r"Mother\d+_4\.fif", f)]
mom_ru = [f for f in os.listdir(thesisPath) if re.match(r"Mother\d+_5\.fif", f)]
...
for participant in sorted(os.listdir(dataPath)):
    ...
    for i, sfp_stage in enumerate(sorted(os.listdir(participantPath))):
        ...
        for surr in range(n_surrogates):
            ...
            # pairing the baby with a random mom (which is definitely not baby's same mom)
...
with open("results_theta_validated_cond.json", "w") as results_file:
    json.dump(plv_results_theta, results_file)
    
with open("results_alpha_validated_cond.json", "w") as results_file:
