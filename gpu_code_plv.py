...
for participant in sorted(os.listdir(dataPath)):
    ...
    for sfp_stage in sorted(os.listdir(participantPath)):
        ...
        stage_dict[stage]={'theta': 
            plv.theta_baby.tolist(), 'alpha': plv.alpha_baby.tolist()}
...
# np.save('theta_matrix_plv', matrix_theta)

# np.save('alpha_matrix_plv', matrix_alpha)
...
#         plv=PLV(baby.epochs, mom.epochs)
#         plv.get_plv()
#         plv_theta = np.mean(np.array(plv.theta_baby), axis = 0)
#         plv_alpha = np.mean(np.array(plv.alpha_baby), axis = 0)
#         stage_dict_theta[stage] = plv.theta_baby.tolist()
#         stage_dict_alpha[stage] = plv.alpha_baby.tolist()
#     plv_results_theta[participant_idx]=stage_dict_theta
#     plv_results_alpha[participant_idx]=stage_dict_alpha
